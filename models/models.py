    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if k == 2 and stride == 1:  # yolov3-tiny
                ...
                modules = maxpool
        elif mdef['type'] == 'Hourglass':
            f = mdef['filters']
            i = mdef["increase"]
            H_n = mdef["n"]
            modules = Hourglass(H_n, f, increase = i)

        elif mdef['type'] == 'Hourglass_CBAM':
            f = mdef['filters']
            i = mdef["increase"]
            H_n = mdef["n"]
            modules = Hourglass_CBAM(H_n, f, increase = i)
        
        elif mdef['type'] == "Residual":
            f = mdef['filters']
            modules = Residual(f,f)

        elif mdef["type"] == "Weighting":
            f = mdef["filters"]
            modules = Weighting(f,f)
        
        elif mdef["type"] == "CBAM":
            ratio = mdef["ratio"]
            inputchannel = mdef["channel"]
            modules = CBAM(inputchannel, ratio)
