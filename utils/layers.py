...
class Residual_CBAM(nn.Module): 
    def __init__(self, inp_dim, out_dim):
        super(Residual_CBAM, self).__init__()
        self.relu = nn.ReLU()
        ...
        self.CBAM1 = CBAM(int(out_dim/2), reduction_ratio=16, pool_types=["avg","max"], no_spatial=False)
        self.CBAM2 = CBAM(int(out_dim/2), reduction_ratio=16, pool_types=["avg","max"], no_spatial=False)
        self.CBAM3 = CBAM(int(out_dim), reduction_ratio=16, pool_types=["avg","max"], no_spatial=False)
        if inp_dim == out_dim:
...
class Hourglass_CBAM(nn.Module): # n < residual depth, f < input filter
    def __init__(self, n, f, bn=None, increase=0):
        super(Hourglass_CBAM, self).__init__()
        nf = f + increase
        self.up1 = Residual_CBAM(f, f)
        # Lower branch
        self.pool1 = Pool(2,2)
        self.low1 = Residual_CBAM(f, nf)
        self.n = n
        # Recursive hourglass
        if self.n > 1:
            self.low2 = Hourglass_CBAM(n-1, nf, bn=bn)
        else:
            self.low2 = Residual_CBAM(nf, nf)
        self.low3 = Residual_CBAM(nf, f)
...
class Conv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride = 1, bn = False, relu = True):
        ...
        #self.conv = nn.Conv2d(inp_dim, out_dim, kernel_size, stride, padding=0, bias=True)
...
class Hourglass(nn.Module): # n < residual depth, f < input filter
    def __init__(self, n, f, bn=None, increase=0):
        ...
        self.up2 = nn.Upsample(scale_factor=2, mode='nearest')
        #self.up2 = nn.Upsample(size=, mode='nearest')
